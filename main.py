...
def main():
    ...
    print('LOADING DATA...')
    if not config["load_data_params"]["pkl_reload"] and not path.exists(f'./Data/{config["load_data_params"]["df_name"]}_binarized.pkl'):
    ...
    if config["load_data_params"]["pkl_reload"]:
        ...
        df.to_pickle(f'./Data/{config["load_data_params"]["df_name"]}_binarized.pkl')
    else:
        print('Data was loaded from pickle')
        df = pd.read_pickle(f'./Data/{config["load_data_params"]["df_name"]}_binarized.pkl')  
    ...
    print(f'df_name: {config["load_data_params"]["df_name"]}  columns_number: {columns_number}  observations_number: {X_train.shape[0]}  columns_number: {columns_number}  formulas_number: {formulas_number}  models_per_formula: {models_per_formula}  total_count_of_models: {total_count}')
    ...
    if config['rules_generation_params']['subset_size'] != 1:
        ...
        best_1_variable, average_time_per_model = find_best_model_parallel_formula_reload(X_train, y_train, file_name = config_1_variable["load_data_params"]["df_name"], **config_1_variable["similarity_filtering_params"],  **config_1_variable["rules_generation_params"])
    ...
    print('\nBEGIN TRAINING...')
    start_time = time()
    find_best_model_parallel_formula_reload(X_train, y_train, file_name = config["load_data_params"]["df_name"], **config["similarity_filtering_params"],  **config["rules_generation_params"])
    
    elapsed_time = time() - start_time
    log_exec(config["load_data_params"]["df_name"], config["similarity_filtering_params"]["sim_metric"], \
